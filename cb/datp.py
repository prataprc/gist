...
matchers = [
  ...
  [ re.compile(r'.*\[Info\] DATP\[->dataport ":([0-9]+)"\] new connection "([^"]+)".*'),
    handler_datp_2 ],
  [ re.compile(r'.*\[Info\] DATP\[->dataport ":([0-9]+)"\] added vbucket ([^-]+)-([^-]+)-([0-9]+).*'),
    handler_datp_3 ],
  [ re.compile(r'.*\[Info\] DATP\[->dataport ":([0-9]+)"\] connection "([^"]+)" closed !'),
    handler_datp_4 ],
  [ re.compile(r'.*\[Info\] DATP\[->dataport ":([0-9]+)"\] ... stopped'),
    handler_datp_5 ],
  [ re.compile(r'.*\[Info\] DATP\[->dataport ":([0-9]+)"\] deleted vbucket ([^-]+)-([^-]+)-([0-9]+).*'),
    handler_datp_6 ],
  [ re.compile(r'(.*\[Info\] KVSender::.*)'),
    handler_marker ],
]
