...
@task
def mb14116():
    print("Make sure that KV is populated with atleast 1M documents")
    ...
    with cd(os.sep.join([cbidxdir, "secondary", "cmd", "cbindex"])), shell_env(GOPATH=env.gopath, GOROOT=env.goroot) :
        ...
        trycmd("go build")
        trycmd(cmdfmt % ("-type list",), v=True)
        c = "-type create -bucket default -index idxl -fields=language"
        trycmd(cmdfmt % (c,))
        trycmd(cmdfmt % ("-type list",), v=True)

        trycmd(cmdfmt % ("-type list",), v=True)
        c = "-type scanAll -bucket default -index idxl"
        trycmd(cmdfmt % (c,))
